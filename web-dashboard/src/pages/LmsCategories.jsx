...
export default function LmsCategories() {
  ...
  const load = useCallback(async () => {
    ...
    catch (err) { setError(err?.response?.data?.message || "Could not load LMS categories.") }
  ...
  async function submit(event) {
    ...
    } catch (err) { setError(err?.response?.data?.message || "Could not save the category.") }
    finally { setSaving(false) }
  }

  async function remove(category) {
    if (!window.confirm(`Delete "${category.name}"?`)) return
    try { await deleteLmsCategory(idOf(category)); await load() }
    catch (err) { setError(err?.response?.data?.message || "Could not delete the category.") }
  }
