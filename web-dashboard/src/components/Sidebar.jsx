...
export default function Sidebar({ mobileOpen = false, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-[240px] min-h-screen bg-slate-900 flex flex-col shrink-0 transition-transform duration-200 ${
      ...
      >
        <div className="flex items-center gap-2.5 px-5 py-5 border-b border-white/[0.07]">
          ...
          <div className="min-w-0">
            <p className="text-slate-100 font-semibold text-sm tracking-tight truncate">Mend Admin</p>
            <p className="text-[10px] text-slate-500 truncate">Platform console</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="ml-auto lg:hidden w-8 h-8 rounded-lg text-slate-400 hover:text-white hover:bg-white/5"
        ...
        <div className="px-3 pb-4 pt-3 border-t border-white/[0.07] space-y-2">
          {(session.email || session.role) && (
            <div className="px-3 py-2 rounded-lg bg-white/[0.04]">
              {session.email && (
                <p className="text-[11px] text-slate-300 truncate" title={session.email}>
                  {session.email}
                </p>
              )}
              {session.role && (
